...
نام پنل : {$marzban_list_get['name_panel']}";
        ...
        // SSH panels: use hardcoded template (not from DB)
        if (in_array($marzban_list_get['type'], ['shahan', 'xpanel', 'rocket_ssh', 'dragon'])) {
            ...
            $ssh_volume_display = (intval($get_invoice['Volume']) == 0) ? $textbotlang['users']['stateus']['Unlimited'] : $get_invoice['Volume'] . ' GB';
            $ssh_days_display = (intval($get_invoice['Service_time']) == 0) ? $textbotlang['users']['stateus']['Unlimited'] : $get_invoice['Service_time'];
            $ssh_expire_date = (intval($get_invoice['Service_time']) == 0) ? $textbotlang['users']['stateus']['Unlimited'] : date('Y/m/d', time() + (intval($get_invoice['Service_time']) * 86400));
            $ssh_connection_limit = 1;
            $info_product_ssh = select("product", "*", "id_product", $get_invoice['id_product'], "select");
            if ($info_product_ssh && isset($info_product_ssh['inbounds']) && is_numeric($info_product_ssh['inbounds'])) {
                $ssh_connection_limit = intval($info_product_ssh['inbounds']);
            }
...
🔐 لینک NPVT :
<code>{$npvt_link}</code>";
            $textcreatuser = $datatextbot['textafterpay'];
...
{
    global $from_id, $optionMarzban, $optionX_ui_single, $optionhiddfy, $optionalireza, $optionalireza_single, $optionmarzneshin, $option_mikrotik, $optionwg, $options_ui, $optioneylanpanel, $optionibsng, $optionssh;
    ...
    } elseif (in_array($typepanel, ['shahan', 'xpanel', 'rocket_ssh', 'dragon'])) {
        sendmessage($from_id, $message, $optionssh, 'HTML');
    }
}
